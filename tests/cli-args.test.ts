...
describe('CLI Argument Validation Tests', () => {
  ...
  test('should reject when --enabled-tools is provided without value', async () => {
    const process = Bun.spawn([controllerExecutable, '--enabled-tools'], {
      stdin: 'pipe',
      stdout: 'pipe',
      stderr: 'pipe',
    });
  ...
  test('should reject when --disabled-tools is provided without value', async () => {
    const process = Bun.spawn([controllerExecutable, '--disabled-tools'], {
      stdin: 'pipe',
      stdout: 'pipe',
      stderr: 'pipe',
    });
  ...
  test('should reject when no target command is provided', async () => {
    const process = Bun.spawn(
      [controllerExecutable, '--enabled-tools', 'add'],
      {
        stdin: 'pipe',
        stdout: 'pipe',
        stderr: 'pipe',
  ...
  test('should accept valid --enabled-tools argument', async () => {
    const process = Bun.spawn(
      [controllerExecutable, '--enabled-tools', 'add,get-args', 'echo', 'test'],
      {
        stdin: 'pipe',
        stdout: 'pipe',
        stderr: 'pipe',
