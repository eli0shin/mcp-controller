...
describe('List Tools Command Tests', () => {
  ...
  test('should list all available tools', async () => {
    const process = Bun.spawn(
      [controllerExecutable, 'list-tools', 'bun', 'run', fixtureServerPath],
      {
        stdin: 'pipe',
        stdout: 'pipe',
        stderr: 'pipe',
  ...
  test('should list only enabled tools when --enabled-tools is specified', async () => {
    ...
    const lines = output.trim().split('\n');
    expect(lines).toEqual(['add: Add two numbers']);
  });
  ...
  test('should show error when no target command specified', async () => {
    const process = Bun.spawn([controllerExecutable, 'list-tools'], {
      stdin: 'pipe',
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    expect(errorOutput.trim()).toBe(
      'Error: No target command specified for list-tools'
    );
  ...
  test('should handle server initialization errors', async () => {
    const process = Bun.spawn(
      [controllerExecutable, 'list-tools', 'nonexistent-command'],
      {
        stdin: 'pipe',
        stdout: 'pipe',
        stderr: 'pipe',
