...
import type { OperationResult, UpdateState } from './update-types.js';

export function getUpdateStatePath(): string {
  const xdgStateHome = process.env.XDG_STATE_HOME;
  if (xdgStateHome) {
    return join(xdgStateHome, 'mcp-controller-update-state');
  }
  return join(homedir(), '.mcp-controller-update-state');
}
